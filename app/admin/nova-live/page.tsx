      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        {step === 'url' && (
          <form onSubmit={handleAnalyze} className="space-y-4">
            <div className="bg-white rounded-2xl border-2 border-purple-100 p-8 shadow-sm space-y-5">

              {/* Header */}
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center text-2xl flex-shrink-0">🔗</div>
                <div>
                  <h2 className="text-xl font-bold">Link + dados do produto</h2>
                  <p className="text-sm text-gray-500 mt-0.5">
                    Cole o link para leitura automática — ou preencha os dados manualmente quando o link for bloqueado
                  </p>
                </div>
              </div>
              ...
              {/* URL input */}
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5 tracking-wider">LINK DO PRODUTO</label>
                <input
                  type="text"
                  value={productUrl}
                  onChange={(e) => setProductUrl(e.target.value)}
                  placeholder="https://site-da-marca.com.br/produto..."
              ...
              {/* Platform compatibility guide — 3 tiers, corrected */}
              <div className="rounded-xl border border-gray-100 divide-y divide-gray-100 text-xs overflow-hidden">
                ...
                <div className="bg-green-50 px-4 py-2.5 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-green-800">Leitura automática</span>
                  <span className="text-green-600 ml-1">— qualidade alta</span>
                </div>
                <div className="px-4 py-2 flex flex-wrap gap-1.5 bg-white">
                  {['Site da marca', 'Blog', 'Shopify'].map(s => (
                    <span key={s} className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">{s}</span>
                  ))}
                </div>
                {/* Tier 2 — partial */}
                <div className="bg-yellow-50 px-4 py-2.5 flex items-center gap-2">
                  <span className="text-yellow-600 font-bold">⚡</span>
                  <span className="font-semibold text-yellow-800">Leitura parcial</span>
                  <span className="text-yellow-600 ml-1">— complemente com dados abaixo</span>
                </div>
                <div className="px-4 py-2 flex flex-wrap gap-1.5 bg-white">
                  {['Magalu', 'Americanas'].map(s => (
                ...
                <div className="bg-red-50 px-4 py-2.5 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-red-800">Link bloqueado</span>
                  <span className="text-red-500 ml-1">— preencha os dados abaixo</span>
                </div>
                <div className="px-4 py-2 pb-3 flex flex-wrap gap-1.5 bg-white">
                  {['TikTok Shop', 'Mercado Livre', 'Amazon', 'Shopee', 'Instagram', 'Shein'].map(s => (
                    <span key={s} className="bg-red-50 text-red-600 px-2 py-0.5 rounded-full font-medium border border-red-100">{s}</span>
                  ))}
                </div>
              </div>

              {/* Proactive warning when blocked URL detected */}
              {(['mercadolivre', 'amazon.com.br', 'shopee.com.br', 'tiktok.com', 'instagram.com', 'shein.com'] as const)
                .some(d => productUrl.toLowerCase().includes(d)) && (
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm">
                  <span className="text-amber-500 flex-shrink-0 font-bold text-base leading-none mt-0.5">!</span>
                  <div>
                    <p className="font-semibold text-amber-900">Esse link é bloqueado para leitura automática.</p>
                    <p className="text-amber-700 mt-0.5">Cole os dados do produto no campo abaixo para garantir uma análise de qualidade.</p>
                  </div>
                </div>
              )}

              {/* Manual data — elevated, always visible */}
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-bold text-gray-500 tracking-wider">DADOS DO PRODUTO</label>
                  <span className="text-xs font-semibold text-purple-700 bg-purple-50 border border-purple-200 px-2 py-0.5 rounded-full">
                    obrigatório para links bloqueados
                  </span>
                </div>
                <textarea
                  value={extraContext}
                  ...
                  rows={5}
                  placeholder={`Cole aqui os dados direto da página do produto:\n\nNome · Preço cheio · Preço live\nIngredientes ou especificações técnicas\nDiferenciais e avaliações de clientes\nPúblico-alvo · Categoria`}
                ...
                <p className="text-xs text-gray-400 mt-1.5">
                  Para TikTok Shop e Mercado Livre: copie título, preço, descrição e avaliações da página e cole aqui. Quanto mais dados, mais preciso o curso.
                </p>
              </div>
            ...
            {analyzing && (
              <div className="bg-purple-50 border border-purple-200 rounded-xl p-5 text-sm">
                ...
                <ol className="space-y-2 list-decimal list-inside text-purple-700">
                  <li>Tentando ler o conteúdo da página do produto</li>
                  <li>Usando os dados informados quando o link estiver bloqueado</li>
                  <li>Claude analisa: diferenciais, público-alvo, dores e forças do produto</li>
                  <li>Mapeia regulação ANVISA e claims permitidos e proibidos</li>
