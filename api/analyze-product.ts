 ...
 *
 * Recebe URL de produto → busca conteúdo → Claude analisa →
 * retorna hero + compliance + product_analysis pré-preenchidos.
 */
...
async function fetchProductContent(url: string): Promise<string> {
  try {
    ...
    // Extrai texto do body (remove scripts, styles, nav, menus)
    const bodyText = html
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    console.log(`[analyze-product] Fetching: ${url}`);
    const pageContent = await fetchProductContent(url);
    console.log(`[analyze-product] Content: ${pageContent.length} chars`);
    ...
    const userPrompt = [
      ...
      extra_context
        ? `\nInformações adicionais fornecidas pela equipe:\n${extra_context}`
        : '',
